 ...
 * the owning record's own `country` (COUNTRY-1, Danny addendum: "provincie moet
 * cascaderen op het gekozen land"). Promoted to shared (Danny 22-07, punt 2):
 * originally candidate-only, now also used by the vacancy Details tab — an entity
 * page must never import another entity's internals (§2), so this lives here.
 *
 ...
 * (the candidate/vacancy stores the province name).
 *
