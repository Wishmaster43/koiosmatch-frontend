 ...
 * (G-13 / AP-CO10). Kept as its own module so existing callers (the shared CustomFieldsTab (vacancy drawer), the
...
export function useVacancyCustomFields() {
  ...
  // those extra fields are simply ignored by the shared CustomFieldsTab (vacancy drawer).
  return {
