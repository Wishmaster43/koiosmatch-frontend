...
// One nav entry — a leaf page or a group with children; icon/soon only apply to top-level items.
...
const navLabel = (t: TFunction, id: string, fallback?: string) =>
  t(`nav.${id.replace(/\./g, '_')}`, { defaultValue: fallback })
...
function resolveVisibleNavItems(items: NavItemData[], auth: AuthContextValue | null, t: TFunction): NavItemData[] {
  return items
...
const MODULE_NAV_ITEMS: NavItemData[] = [
  {
...
function SubNavItem({ item, active, onNavigate }: SubNavItemProps) {
...
function NavItem({ item, activePage, expanded, openItems, toggleOpen, onNavigate }: NavItemProps) {
    ...
    <div>
      ...
      {hasChildren && isOpen && expanded && (
        <div className="mb-1" style={{ paddingLeft: 10 }}>
          {item.children?.map((child: NavItemData) => (
...
export default function Sidebar({ expanded, activePage, setActivePage, koiosOpen, onToggleKoios }: SidebarProps) {
  const { t } = useTranslation('common')
