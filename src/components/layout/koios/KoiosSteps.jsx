...
export default function KoiosSteps({ steps, t }) {
      ...
      {open && (
        <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 5 }}>
          {steps.map((s, i) => {
            ...
            const Icon = refused ? meta.Icon : Check
            const color = refused ? meta.color : 'var(--color-success)'
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 6, fontSize: 11 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {refused && <div style={{ color, marginTop: 1 }}>{t(meta.labelKey)}</div>}
                </div>
              </div>
            )
          })}
