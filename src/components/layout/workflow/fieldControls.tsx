...
import { Plus, X, Check, Copy, ChevronUp, ChevronDown } from 'lucide-react'
...
// CMBE K-193 fase 0: GET /whatsapp-phone-numbers options now carry a `coexistence`
// flag. When the sibling `channel` field is 'waba_coex' the list is filtered to
// Coexistence-only numbers; the currently stored value stays visible even if it
// falls outside that filter (§3 no silent clear of a saved value).
...
// ── Ordered-list field ──────────────────────────────────────────────────────────
// WA-SEND-FIELDS-2: whatsapp_send's `body_parameters` (WhatsAppSendModule::
// configSchema, type 'ordered_list') — an ORDER-sensitive list of {value} rows
// (a WhatsApp Flow's positional {{1}}, {{2}}, …), so reordering is a real
// affordance here, unlike the unordered 'keyvalue' field above.
