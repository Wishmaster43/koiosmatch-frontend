/**
 * NoteActionsResultsPanel — the "Actiepunten" result surface (K0-B, F4).
 * Before "Uitvoeren": the plain suggested-items preview + three actions
 * (Uitvoeren / Als tekst toevoegen — the OLD append-as-list apply, kept as a
 * secondary option / Verwerpen). After "Uitvoeren": one NoteActionItemCard per
 * item with its live execute status, a Klaar close, and — for an executed
...
import { fetchWorkflowRun } from './noteActionsExecuteApi'
import { useNoteActionsExecute } from './useNoteActionsExecute'
import NoteActionItemCard from './NoteActionItemCard'
...
interface NoteActionsResultsPanelProps {
  // The suggested items from the note-assist 'actions' result (unchanged).
  items: AssistActionItem[]
  // Existing note being edited → its id, for the execute request's provenance
  // (`source.note_id`); a new unsaved note passes undefined (no source sent).
  noteId?: string
  // The OLD apply semantics (append the items as a bullet list into the note
  // body) — kept as the secondary option, mirrors the previous "Overnemen".
  onApplyAsText: () => void
  // Discard the whole suggestion — mirrors the previous "Verwerpen".
  onDiscard: () => void
}

export default function NoteActionsResultsPanel({ items, noteId, onApplyAsText, onDiscard }: NoteActionsResultsPanelProps) {
  const { t } = useTranslation('common')
  const exec = useNoteActionsExecute(noteId)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {exec.items.map((it, i) => (
        <NoteActionItemCard key={i} item={it}
