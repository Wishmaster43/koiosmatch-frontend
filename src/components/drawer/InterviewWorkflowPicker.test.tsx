...
// WFB-08: WorkflowResource never emits a folder object, so option labels are
// plain workflow names (no "Kelly · " prefix).
const options = [
  { value: 'wf-1', label: 'Kelly-Helpende' },
  { value: 'wf-2', label: 'Kelly-Verpleegkundige' },
]

describe('InterviewWorkflowPicker · options + clear (VAC-CLEAR-1)', () => {
  it('renders the plain option labels and emits the picked id', async () => {
...
describe('InterviewWorkflowPicker · four UI states', () => {
  ...
  it('never shows the empty-list notice once a workflow is actually linked', () => {
    render(<InterviewWorkflowPicker value="wf-1" onChange={vi.fn()} options={[]}
      linkedRef={{ id: 'wf-1', name: 'Zorgintake', agent: null }} />)
...
describe('InterviewWorkflowPicker · linked-but-missing fallback (verdict finding 2)', () => {
  it('seeds a fallback option from the nested ref when the linked workflow is missing from options', () => {
    // WFB-08: InterviewWorkflowRef has no folder field — only the plain name renders.
    render(<InterviewWorkflowPicker value="wf-9" onChange={vi.fn()} options={options}
      linkedRef={{ id: 'wf-9', name: 'Zorgintake', agent: null }} />)
    // The trigger shows the resolved name, never the raw id.
    expect(screen.getByRole('button', { name: 'Zorgintake' })).toBeInTheDocument()
  ...
  it('resolves a linked-but-inactive workflow via describe(), with the inactive marker + warning', () => {
    const describeInactive = (id: string) => (id === 'wf-9' ? { label: 'Zorgintake', inactive: true } : null)
    render(<InterviewWorkflowPicker value="wf-9" onChange={vi.fn()} options={options} describe={describeInactive} />)
    expect(screen.getByRole('button', { name: 'Zorgintake aiagent.workflow.inactiveSuffix' })).toBeInTheDocument()
