...
/**
 * OpportunityChangelogPopover — a History icon in the drawer title-row that opens
 * the opportunity's audit trail in a popover. Mirrors the candidate ChangelogPopover:
 * record history stays one click from every tab instead of being its own tab.
 */
export default function OpportunityChangelogPopover({ opportunity }: { opportunity: Opportunity }) {
      ...
      {open && (
        <div role="dialog" aria-label={t('drawer.tabs.changelog')}
          ...
          <div style={{ overflowY: 'auto', padding: '12px 14px' }}>
            <ChangelogTab opportunity={opportunity} />
          </div>
        </div>
      )}
    </div>
