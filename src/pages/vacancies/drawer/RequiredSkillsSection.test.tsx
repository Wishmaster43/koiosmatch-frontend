/**
 * RequiredSkillsSection · VACANCY-SKILLS-PARITY-1 (Danny 08-08) regression
...
describe('RequiredSkillsSection · list rendering (§3B: vertical list, never chips)', () => {
  it('renders every skill as its own row, each with its OWN pencil + trash', () => {
    render(<RequiredSkillsSection skills={['Triage', 'Wondzorg']} onAddSkill={vi.fn()} onEditSkill={vi.fn()} onRemoveSkill={vi.fn()} />)
  ...
  it('shows the empty-state text and the add trigger when there are no skills yet', () => {
    render(<RequiredSkillsSection skills={[]} onAddSkill={vi.fn()} onEditSkill={vi.fn()} onRemoveSkill={vi.fn()} />)
...
describe('RequiredSkillsSection · add flow (same idiom as the candidate SkillsTab)', () => {
  ...
  it('saving the add form calls onAddSkill with the trimmed typed name', async () => {
    ...
    render(<RequiredSkillsSection skills={[]} onAddSkill={onAddSkill} onEditSkill={vi.fn()} onRemoveSkill={vi.fn()} />)
    await user.click(screen.getByRole('button', { name: /details\.addSkill/ }))
...
describe('RequiredSkillsSection · per-row edit (real rename, not remove+re-add)', () => {
  ...
  it('saving the edit form calls onEditSkill with (index, newName) — the row keeps its position', async () => {
    ...
    render(<RequiredSkillsSection skills={['Triage', 'Wondzorg']} onAddSkill={vi.fn()} onEditSkill={onEditSkill} onRemoveSkill={onRemoveSkill} />)
    await user.click(screen.getAllByTitle('Bewerken')[1])
...
describe('RequiredSkillsSection · per-row remove', () => {
  it('the row trash calls onRemoveSkill with THAT row\'s value, not an index or the whole list', async () => {
    const user = userEvent.setup()
    const onRemoveSkill = vi.fn()
    render(<RequiredSkillsSection skills={['Triage', 'Wondzorg']} onAddSkill={vi.fn()} onEditSkill={vi.fn()} onRemoveSkill={onRemoveSkill} />)
