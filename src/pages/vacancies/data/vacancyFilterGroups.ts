...
interface BuildArgs {
  ...
  // The organisation-level pickers (owner/client/category/branch) — state and
  // options live on the page (insights-derived), the BUILDER owns their order:
  // categories must be CONTIGUOUS or the shared sidebar renders a heading twice
  // (Opus filterpaneel-verify: Organisatie en Algemeen renderden elk 2×).
  org: {
    owner:    { selected: string[]; options: Opt[]; onToggle: Toggle }
    client:   { selected: string[]; options: Opt[]; onToggle: Toggle }
    category: { selected: string[]; options: Opt[]; onToggle: Toggle }
    branch:   { selected: string[]; options: Opt[]; onToggle: Toggle }
  }
}
...
export function buildVacancyFilterGroups({ t, filters: f, options: o, org }: BuildArgs) {
  ...
  return [
    ...
    // ── Organisation (contiguous, one heading): owner · client · category · agent · branch ──
    { key: 'owner',    type: 'search-select', category: catOrg, label: t('filters.owner'),    selected: org.owner.selected,    options: org.owner.options,    onToggle: org.owner.onToggle },
    { key: 'client',   type: 'search-select', category: catOrg, label: t('filters.client'),   selected: org.client.selected,   options: org.client.options,   onToggle: org.client.onToggle },
    { key: 'category', type: 'search-select', category: catOrg, label: t('filters.category'), selected: org.category.selected, options: org.category.options, onToggle: org.category.onToggle },
