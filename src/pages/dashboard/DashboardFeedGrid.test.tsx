/**
 * DASH-FEEDS-PACK-1 / DASH-PAIRS-1 — the work-feed tiles and the recent lists
 * share ONE packed grid each, so a tile that hid itself never leaves a hole
 ...
 * `vis` is mocked to always return true below (the same shape admin/
 * management's '*' wildcard produces) specifically to prove that even under a
 * wildcard visibility predicate, the three removed titles never render — the
 * resilience has to live in the registry, since the '*' wildcard by design
 * matches ANY id (§ measured map step 7).
 */
...
// actually RENDER is decided by their own data, exactly as in the app.
vi.mock('./hooks/useDashboardViewModel', () => ({
  useDashboardViewModel: () => ({
    ...
    // Only two of the three widget feeds have data — koiosSuggestions stays
    // empty and self-hides, which is the situation that used to leave holes.
...
describe('Dashboard work-feed grid (DASH-FEEDS-PACK-1)', () => {
  it('packs the two tiles that have data into the same grid, with no hole between them', () => {
    ...
    // DOM path: primary-text div → row div → WidgetListBlock's Block-outer div →
    // the grid cell → the grid.
    const grid = expiring.parentElement!.parentElement!.parentElement!.parentElement!.parentElement!
    // Two-column grid …
  ...
  // if a removed block had survived in the registry, it would render here.
