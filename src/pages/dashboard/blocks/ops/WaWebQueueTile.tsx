...
export default function WaWebQueueTile({ feed, onNavigate }: {
  ...
  const figures: { key: string; label: string; value: number; danger?: boolean }[] = [
    { key: 'in_queue', label: t('feed.waWebQueue.inQueue'), value: feed.in_queue },
    { key: 'sending', label: t('feed.waWebQueue.sending'), value: feed.sending },
    { key: 'failed', label: t('feed.waWebQueue.failed'), value: feed.failed, danger: feed.failed > 0 },
  ]
