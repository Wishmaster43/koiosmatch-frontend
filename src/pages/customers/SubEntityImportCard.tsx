 ...
 * source) plus an explicit warning naming the open customer, via
 * EntityImportCard's `introExtra` slot, and (b) once the dry run answers, every
 * resolved row's `reference` ("CustomerName / …") is checked against
 * `customerName` — a mismatch renders EntityImportCard's `previewBanner` and
 * blocks the plain Confirm button behind an explicit acknowledgement dialog
 * (`onConfirmImport`) naming the other customer and the row count, so nobody
 * imports hundreds of rows into the wrong client while believing the open
 * drawer scoped it.
 */
...
import type { ImportRowResult } from '@/pages/settings/shared'
...
// Wraps the shared EntityImportCard with the parent-mismatch copy/banner/gate
// (see the file header): the wizard's own preview result drives all three.
export default function SubEntityImportCard({ entity, customerName, wizard, canView, canImport }: SubEntityImportCardProps) {
  ...
  // The real import is one click away in EntityImportCard's preview step —
  // intercepted here so a parent mismatch asks first, naming the other
  // customer and the row count. No mismatch: fall straight through to the
  // dry-run-confirmed real POST.
  const handleConfirm = () => {
    if (mismatch) {
      confirm(
  ...
  return (
    <>
      <EntityImportCard
        ...
        canImport={canImport}
        entity={entity}
        intro={t('subModal.import.intro', { entity: entityLabel })}
        ...
        previewBanner={mismatch && (
          ...
          <div role="alert" style={{ display: 'flex', gap: 8, alignItems: 'flex-start', padding: '10px 12px',
            background: tintBg('var(--color-danger)'),
