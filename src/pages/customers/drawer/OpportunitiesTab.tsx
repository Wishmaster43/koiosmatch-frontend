...
import { opportunityValueOf, formatOpportunityValue } from '@/pages/opportunities/shared'
  ...
  const columns: Column<Opportunity>[] = [
    ...
    { key: 'actions', header: '', align: 'right', render: o => (
      <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
        <Button variant="secondary" iconOnly size="sm" onClick={e => { e.stopPropagation(); setEditingOpp(o) }} title={t('common:edit')} aria-label={t('common:edit')}>
          <Pencil size={12} />
        </Button>
        <Button variant="dangerSoft" iconOnly size="sm" onClick={e => { e.stopPropagation(); remove(o) }} title={t('common:delete')} aria-label={t('common:delete')}>
          <Trash2 size={12} />
        </Button>
      </div>
    ) },
