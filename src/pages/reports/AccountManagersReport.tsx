...
export default function AccountManagersReport({ period, filters = EMPTY_REPORT_FILTERS }: { period: ReportPeriod; filters?: ReportFilterState }) {
  ...
  const kpis: KpiSpec[] = [
    { key: 'accountManagers', label: t('accountmanagers.summary.accountManagers'), value: managerCount },
    { key: 'customers', label: t('accountmanagers.summary.customersInWindow'), value: totalCustomers },
    { key: 'avgPerManager', label: t('accountmanagers.summary.avgPerManager'), value: formatNumber(avgPerManager) },
    { key: 'topManager', label: t('accountmanagers.summary.topManager'),
      value: topManager ? `${topManager.name} · ${topManager.count}` : '—' },
    { key: 'openOpportunities', label: t('accountmanagers.summary.openOpportunities'), value: '—' },
    { key: 'activeMatches', label: t('accountmanagers.summary.activeMatches'), value: '—' },
    { key: 'revenue', label: t('accountmanagers.summary.revenue'), value: '—' },
    { key: 'renewalsDue', label: t('accountmanagers.summary.renewalsDue'), value: '—' },
    { key: 'notContacted', label: t('accountmanagers.summary.notContacted'), value: '—' },
  ]
