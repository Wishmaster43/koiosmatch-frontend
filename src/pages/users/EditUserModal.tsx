...
// SETTINGS-INCON-B2 (Danny 13-09, "Pop-up wijzigen gebruiker is te smal … Naam
// niet leesbaar"): the wide-form footprint every create modal shares (mirrors
// AddCandidateModal) — the old 420px panel squeezed firstname/lastname into an
// unreadable ~180px column each.
...
export default function EditUserModal({ user, onClose, onSaved }: {
  ...
  return (
    // SETTINGS-INCON-B2: the wide-form footprint (mirrors AddCandidateModal),
    // replacing the old 420px single column that made the name fields unreadable.
    <FloatingPanel open onClose={onClose} ariaLabel={t('editUser')}
      persistKey="edit-user" {...WIDE_MODAL_PANEL_SIZE} bodyStyle={{ padding: '20px 24px 24px' }}
      header={
        ...
        <form onSubmit={handleSubmit}>
          <UserModalColumns
            left={<>
              {/* Personal — first/last name, shared with NewUserModal (UserNameCard). */}
            ...
            right={
              ...
              <div>
                <div style={cardHead}>{t('branches.title')}</div>
                <div style={{ ...cardBox, position: 'relative' }}>
                  {branchesSaving && (
                    <span style={{ position: 'absolute', top: 12, right: 12, color: 'var(--text-muted)' }}><Spinner size={12} /></span>
                  )}
                  <Caption as="p">{t('branches.hint')}</Caption>
                  {branchesLoading ? (
                  ...
                  {!branchesLoading && !branchesError && branches.length === 0 && locationOptions.length > 0 && (
                    <Caption as="p">{t('branches.emptyHint')}</Caption>
                  )}
