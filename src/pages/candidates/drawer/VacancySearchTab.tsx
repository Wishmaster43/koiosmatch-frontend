...
function VacancySearchTabInner({ candidate }: { candidate: Candidate }) {
  ...
  const { statuses: statusOptions } = useVacancyLookups()
  ...
  const openVacancy = (id: Id) => openEntity('vacancies', id)
  ...
  const listPane: ReactNode = loading ? (
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      {rows.map(r => (
        ...
        <button key={String(r.id)} type="button" onClick={() => openVacancy(r.id)}
