/**
 * CandidatesInsightsRow — one compact strip that replaces the old KPI tiles +
 * donut charts. Every card has the SAME footprint (equal width via flex:1, fixed
 * height) so donuts and KPI numbers line up on a single row.
 *
 *   donuts: Array<{ key, title, data, colors?, onPick }>   → MiniDonut cards
 *   kpis:   Array<{ key, label, value, sub?, color, onClick?, active?, channels? }>
 *
 * `channels` (optional) renders a tiny channel breakdown under a KPI value
 * (used by the "active conversations" card: WhatsApp / e-mail / RC).
 */
...
export default function CandidatesInsightsRow({ donuts = [], kpis = [] }) {
  return (
