/**
 * buildCandidateFilterGroups — regression guard for the DATUM-1 zero-pad fix
...
import { buildCandidateFilterGroups } from './candidateFilterGroups'
import type { DateRangeFilter } from '../hooks/useCandidateFilters'

// Mirrors CandidatesPage's own `tog` (generic multi-select toggle updater).
const tog = <T,>(set: (fn: (p: T[]) => T[]) => void) => (v: T) => set(p => p.includes(v) ? p.filter(x => x !== v) : [...p, v])
...
function build(dateRange: DateRangeFilter | null) {
  const t = i18n.getFixedT('nl', 'candidates')
  return buildCandidateFilterGroups({
    t, tog,
    filters: {
      ...
      attentionFilter: null, setAttentionFilter: vi.fn(),
...
describe('buildCandidateFilterGroups · period chip zero-pads single-digit day/month (DATUM-1)', () => {
