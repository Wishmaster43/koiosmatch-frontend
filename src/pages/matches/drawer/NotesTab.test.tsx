...
// tab fetches its own list (GET /matches/{id}/notes), so unwrapList must be real.
...
// OPTIMISTIC-REVERT-1 pattern (mirrors vacancies/applications NotesTab.test.tsx):
...
const match = (over: Partial<MatchRow> = {}) => ({
  id: 1, owner: 'Bente de Jong', ...over,
} as unknown as MatchRow)

describe('match NotesTab (NT-MATCH-1, shared reuse)', () => {
  it('fetches this match\'s notes and shows the empty state', async () => {
    render(<NotesTab match={match()} />)
    await waitFor(() => expect(mockGet).toHaveBeenCalledWith('/matches/1/notes'))
  ...
  // MatchNoteController validates `type` against the entity=match note_types scope
  // (NT-MATCH-1), so the composer must actually carry the picked type on save.
  it('POSTs the picked note type to /matches/{id}/notes', async () => {
    ...
    await waitFor(() => expect(mockPost).toHaveBeenCalledWith('/matches/1/notes', expect.objectContaining({ type: 'intake' })))
  })
