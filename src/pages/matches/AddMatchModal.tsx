...
// Field label + shared searchable single-select (pick-only). Mirrors AddApplicationModal.
...
/**
 * AddMatchModal — create a *direct* match by linking an existing candidate to a
 * vacancy (§3B "direct match" path, no funnel). Pickers load from /candidates and
 * /vacancies. Persists to POST /matches; on failure it still adds the row locally
 * so the flow works before the endpoint is live (forward-compatible).
 */
export default function AddMatchModal({ onClose, onCreated }: { onClose: () => void; onCreated: (m: MatchRow) => void }) {
  ...
  // Build the flat MatchRow the table renders from the picked candidate + vacancy.
  const buildRow = (id: Id, cand?: PickOption, vac?: PickOption): MatchRow => ({
    id,
  ...
  // Create the match; optimistic local row if the endpoint isn't live yet.
  const create = async () => {
    ...
    try {
      const res = await api.post('/matches', { candidate_id: candidateId, vacancy_id: vacancyId })
      const created = (res.data?.data ?? res.data) as { id?: Id } | undefined
      onCreated(buildRow(created?.id ?? -Date.now(), cand, vac))
    } catch {
      onCreated(buildRow(-Date.now(), cand, vac))
    } finally {
      setSaving(false)
      onClose()
    }
    ...
    <>
      ...
      <div ref={panelRef} role="dialog" aria-modal="true" aria-label={t('add.title')} tabIndex={-1}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '14px 22px', borderTop: '1px solid var(--border)' }}>
          <button onClick={onClose} style={{ height: 36, padding: '0 16px', fontSize: 13, border: '1px solid var(--border)', borderRadius: 8, background: 'var(--surface)', color: 'var(--text)', cursor: 'pointer' }}>{t('add.cancel')}</button>
