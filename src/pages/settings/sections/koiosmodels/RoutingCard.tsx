...
import type { KoiosRequestType, KoiosModelsAdminData } from './types'
...
export default function RoutingCard({ data, onSaved }: { data: KoiosModelsAdminData; onSaved: (patch: Partial<KoiosModelsAdminData>) => void }) {
  ...
  const { draft, setDraft, saving, saved, error, dirty, save } = useCardDraft(
    data.routing,
    (draft) => patchKoiosModelsAdmin({ routing: draft }),
    (draft, original) => REQUEST_TYPES.some(rt => JSON.stringify(draft[rt]) !== JSON.stringify(original[rt])),
    (result) => onSaved({ routing: result.routing }),
  )
