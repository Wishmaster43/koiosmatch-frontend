...
// One month's row in the history array (AdminUsageController@show's history[]).
type MonthHistoryRow = NonNullable<AdminTenantUsage['history']>[number]

interface TenantUsageDetailsTableProps {
  // The tenant's monthly usage history, as returned alongside the selected-month summary.
  history?: MonthHistoryRow[] | null
}
...
function monthLabel(value: string | undefined, locale: string): string {
  if (!value) return '—'
...
export default function TenantUsageDetailsTable({ history }: TenantUsageDetailsTableProps) {
  ...
  const columns: Column<MonthHistoryRow>[] = [
    { key: 'month', header: t('usage.details.colMonth'), render: (r: MonthHistoryRow) => <span style={{ textTransform: 'capitalize' }}>{monthLabel(r.month, locale)}</span> },
    { key: 'aiTokens', header: t('usage.details.colAiTokens'), align: 'right', render: (r: MonthHistoryRow) => formatNumber(r.ai?.tokens) },
    { key: 'aiCalls', header: t('usage.details.colAiCalls'), align: 'right', render: (r: MonthHistoryRow) => formatNumber(r.ai?.requests) },
    { key: 'workflowRuns', header: t('usage.details.colWorkflowRuns'), align: 'right', render: (r: MonthHistoryRow) => formatNumber(r.workflow_tokens?.total_module_runs) },
    { key: 'totalAmount', header: t('usage.details.colTotalAmount'), align: 'right', render: (r: MonthHistoryRow) => formatCurrency(r.billing?.total_amount) },
  ]
  ...
  function renderExpanded(row: MonthHistoryRow) {
    const connectors = Array.isArray(row.connectors) ? row.connectors.filter((c) => (c.usage ?? 0) > 0) : []
  ...
  // Bounded height (Danny 17-08: "how long
    ...
    <>
      ...
      >
        <DataTable
          columns={columns}
          rows={rows}
          getRowId={(r: MonthHistoryRow) => r.month}
          stickyHeader
