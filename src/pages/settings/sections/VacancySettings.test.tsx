...
// A fixture lookup row, overridable per test.
interface RowFixture { id: string; name: string; color: string; is_default: boolean; icon?: string }
// eslint-disable-next-line no-restricted-syntax -- DATA: a fixture row's tenant-picked colour, not a style rule.
const row = (over: Partial<RowFixture> = {}): RowFixture => ({ id: 'r1', name: 'Medior', color: '#6FA8C4', is_default: false, ...over })
...
describe('VacancyStatusSettings', () => {
  interface StatusFixture { id: string; name: string; color: string; is_open: boolean; is_closed: boolean; is_default: boolean }
  // eslint-disable-next-line no-restricted-syntax -- DATA: a fixture row's tenant-picked colour, not a style rule.
  const status = (over: Partial<StatusFixture> = {}): StatusFixture => ({ id: 'st1', name: 'Open', color: '#79B58E', is_open: false, is_closed: false, is_default: false, ...over })
...
describe('VacancyChannelSettings', () => {
  interface ChannelFixture { id: string; name: string; color: string; icon: string; active: boolean; default_enabled: boolean; locked_on?: boolean }
  // eslint-disable-next-line no-restricted-syntax -- DATA: a fixture row's tenant-picked colour, not a style rule.
  const channel = (over: Partial<ChannelFixture> = {}): ChannelFixture => ({ id: 'ch1', name: 'Indeed', color: '#6E8FD6', icon: 'globe', active: true, default_enabled: true, ...over })
...
describe('vacancy statuses and phases — icon-and-colour mark (LOOKUP-ICONS-FE-2)', () => {
  interface Case { name: string; Comp: () => ReactElement; endpoint: string; label: string }
  const cases: Case[] = [
