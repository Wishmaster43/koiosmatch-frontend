...
type AppFieldSettings = Record<string, AppFieldValue>
...
export function VacancyApplicationDefaultsSettings() {
  ...
  const setField = (f: string, val: AppFieldValue) => setDraft(d => ({ ...d, [f]: val }))
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
        {APP_FIELDS.map(f => (
          <div key={f} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            ...
            <div style={{ width: 160 }}>
              <SelectMenu value={draft[f] ?? 'optional'} options={valueOptions} onChange={(val: string) => setField(f, val as AppFieldValue)} menuWidth={160} />
            </div>
          </div>
        ))}
      </div>
