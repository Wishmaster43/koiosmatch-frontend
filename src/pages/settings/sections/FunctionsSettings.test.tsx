...
async function renderWithFunctions(rows: Array<Record<string, unknown>> = [], allowFreeEntry = false) {
  vi.resetModules()
...
describe('FunctionsSettings — icon mark (LOOKUP-ICONS-FE-2 fix, 13-09)', () => {
  it('renders the icon-only mark (no colour column) and picking an icon PUTs {icon}', async () => {
    ...
    await waitFor(() => expect(mockedApi.put).toHaveBeenCalledWith('/functions/fn1', expect.objectContaining({ icon: expect.any(String) })))
  })
})
