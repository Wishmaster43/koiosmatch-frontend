...
describe('FacebookLeadsSettings — saving', () => {
  it('POSTs the plain keys, omitting untouched secrets (never overwrites with empty)', async () => {
    mockedApi.get.mockResolvedValue({ data: { facebook_verify_token: '••••••••', facebook_app_secret: '••••••••', facebook_dataset_id: 'ds-1' } })
    render(<FacebookLeadsSettings />)
    ...
    await waitFor(() => expect(mockedApi.post).toHaveBeenCalledWith('/settings', { facebook_dataset_id: 'ds-1' }))
  })

  it('sends a newly typed verify token like any other secret', async () => {
    mockedApi.get.mockResolvedValue({ data: { facebook_verify_token: '••••••••' } })
    render(<FacebookLeadsSettings />)
    ...
    await waitFor(() => expect(mockedApi.post).toHaveBeenCalledWith('/settings', expect.objectContaining({ facebook_verify_token: 'fresh-verify-token' })))
  })
  ...
  it('sends a newly typed secret, then clears the field back to the masked state', async () => {
    ...
    await waitFor(() => expect(mockedApi.post).toHaveBeenCalledWith('/settings', expect.objectContaining({
