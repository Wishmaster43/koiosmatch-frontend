...
const labelStyle: CSSProperties = { fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }
// Canon field style (G33/fieldMetrics) — was its own padding-6/radius-6 copy.
const inputStyle = fieldInputStyle
const hintStyle: CSSProperties = { fontSize: 11, color: 'var(--text-muted)', marginTop: 3 }

// One generation profile's matcher (which vacancies it applies to) — empty array = matches any value.
...
// One generation profile's content rules (template + prompt-engine parameters).
...
// The full draft this editor renders — mirrors VacancyGenerationProfilesList's emptyDraft().
...
// A reusable content block from the sibling tab (id/name/kind), fed into the "blocks" picker.
interface ContentBlock { id: string; name: string; kind: string }

interface VacancyGenerationProfileEditorProps {
  // The profile being edited (create draft or an opened edit card).
  draft: ProfileDraft
  // Shallow-merges a top-level patch into the parent's draft state.
  onChange: (patch: Partial<ProfileDraft>) => void
  // Reusable blocks available to pick from (sibling Herbruikbare-blokken tab).
...
function MatcherField({ label, options, selected, onToggle, emptyText }: MatcherFieldProps) {
  return (
    <div>
...
export default function VacancyGenerationProfileEditor({ draft, onChange, contentBlocks = [] }: VacancyGenerationProfileEditorProps) {
  ...
  const setMatcher = <K extends keyof ProfileMatcher>(key: K, value: ProfileMatcher[K]) => onChange({ matcher: { ...draft.matcher, [key]: value } })
  const setContent = <K extends keyof ProfileContent>(key: K, value: ProfileContent[K]) => onChange({ content: { ...draft.content, [key]: value } })
  ...
  const removeWord = (w: string) => setContent('forbidden_words', draft.content.forbidden_words.filter(x => x !== w))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div>
        ...
        <input value={draft.name} onChange={(e: ChangeEvent<HTMLInputElement>) => onChange({ name: e.target.value })}
      ...
      <div style={{ maxWidth: 160 }}>
        ...
        <input type="number" min={1} value={draft.priority}
          onChange={(e: ChangeEvent<HTMLInputElement>) => onChange({ priority: Number(e.target.value) || 1 })} style={inputStyle} />
      ...
      <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <div>
            ...
            <SearchSelect
              ...
              onToggle={(v: string) => setContent('tone_of_voice', v)}
              closeOnToggle
              searchable={false}
          ...
          <div>
            ...
            <SearchSelect
              ...
              onToggle={(v: string) => setContent('length', v)}
              closeOnToggle
              searchable={false}
              renderTrigger={(toggle: () => void) => (
                // eslint-disable-next-line huisstijlLegacy/no-restricted-syntax -- SearchSelect renderTrigger: form-field face from fieldMetrics, not a Button
          ...
          <div>
            ...
            <SearchSelect
              ...
              onToggle={(v: string) => setContent('language', v)}
              closeOnToggle
              renderTrigger={(toggle: () => void) => (
                // eslint-disable-next-line huisstijlLegacy/no-restricted-syntax -- SearchSelect renderTrigger: form-field face from fieldMetrics, not a Button
                <button type="button" onClick={toggle} style={{ ...inputStyle, cursor: 'pointer', textAlign: 'left' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, maxWidth: 320 }}>
            <input value={wordDraft} onChange={(e: ChangeEvent<HTMLInputElement>) => setWordDraft(e.target.value)} onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => { if (e.key === 'Enter') { e.preventDefault(); addWord() } }}
        ...
        <div>
          ...
          <ChipMultiSelect
            ...
            onToggle={(v: string) => setContent('content_block_ids', toggle(draft.content.content_block_ids, v))}
