...
export default function NationalitiesSettings() {
  ...
  const onWorkPermitToggle = async (v: boolean) => {
    try {
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <StatusListEditor withColor={false} withIcon title={t('nationalities.title')} subtitle={t('nationalities.subtitle')}
        ...
        rowPrefix={(item: StatusListItem) => item.country_code ? (
          <span aria-hidden="true" style={{ fontSize: 15, lineHeight: 1 }}>{getFlagEmoji(item.country_code as string)}</span>
        ) : null} />
    </div>
  )
}
