...
import type { StatusListItem } from './statusListEditorTypes'

// The entities a note type can be scoped to (registry.jsx nt_* tabs), or null for the General tab.
type NoteEntity = 'candidate' | 'application' | 'customer' | 'location' | 'department' | 'contact' | 'opportunity' | 'vacancy' | 'match' | 'task' | null

interface NoteTypesSettingsProps {
  // Which entity this tab is scoped to; null renders the General (global) tab.
  entity?: NoteEntity
}
...
export default function NoteTypesSettings({ entity = null }: NoteTypesSettingsProps) {
  ...
  const postFilter = useCallback((row: StatusListItem) => (entity ? (row.entity ?? null) === entity : !row.entity), [entity])
  return (
