...
const loadSettings = vi.hoisted(() => vi.fn(async () => ({})))
const saveSettings = vi.hoisted(() => vi.fn(async () => {}))
vi.mock('../lib/settingsApi', () => ({ loadSettings: (...a) => loadSettings(...a), saveSettings: (...a) => saveSettings(...a) }))
...
function withLocationSpy() {
  ...
  window.location = { ...original, href: original.href, search: '', pathname: '/instellingen', hash: '' }
...
describe('EmailSettings · Ontkoppelen (DL-10)', () => {
  it('calls DELETE /settings/email/oauth/{context} and refetches status', async () => {
    ...
    await waitFor(() => expect(api.delete).toHaveBeenCalledWith('/settings/email/oauth/klanten'))
  })
})
