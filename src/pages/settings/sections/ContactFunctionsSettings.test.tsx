...
describe('ContactFunctionsSettings', () => {
  it('GETs /contact-functions on mount with no params', async () => {
    ...
    await waitFor(() => expect(mockedApi.get).toHaveBeenCalledWith('/contact-functions?active=1', undefined))
  })
  ...
  it('renders the full editor with items when the endpoint responds normally', async () => {
    mockedApi.get.mockResolvedValue({ data: [{ id: 'f1', name: 'Locatiemanager' }] })
    render(<ContactFunctionsSettings />)
...
async function renderWithContactFunctions(rows: Array<Record<string, unknown>> = [], allowFreeEntry = true) {
  vi.resetModules()
  const apiModule = await import('@/lib/api')
  // vi.mocked() gives the mocked-module factory's plain vi.fn()s their real Mock typing.
  const mockedApi = vi.mocked(apiModule.default, true)
