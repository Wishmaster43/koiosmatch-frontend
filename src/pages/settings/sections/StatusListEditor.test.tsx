...
interface TypeFixture { id: string; name: string; color: string; is_default: boolean; in_use?: boolean }
// eslint-disable-next-line no-restricted-syntax -- DATA: a fixture type's tenant-picked colour, not a style rule.
const type = (over: Partial<TypeFixture> = {}): TypeFixture => ({ id: 't1', name: 'Intake', color: '#3B8FD4', is_default: false, ...over })
...
describe('StatusListEditor — multiple independent defaultFields', () => {
  // eslint-disable-next-line no-restricted-syntax -- DATA: a fixture type's tenant-picked colour, not a style rule.
  const twoFlagType = (over: Partial<TypeFixture & { is_default_for_application: boolean }> = {}) => ({ id: 't1', name: 'Intake', color: '#3B8FD4', is_default: false, is_default_for_application: false, ...over })
...
describe('StatusListEditor — multiple independent flagFields', () => {
  interface FlagRowFixture { id: string; name: string; color: string; is_won: boolean; is_lost: boolean }
  // eslint-disable-next-line no-restricted-syntax -- DATA: a fixture row's tenant-picked colour, not a style rule.
  const twoFlagRow = (over: Partial<FlagRowFixture> = {}): FlagRowFixture => ({ id: 's1', name: 'Won', color: '#3B8FD4', is_won: false, is_lost: false, ...over })
...
describe('StatusListEditor — delete failures notify the user', () => {
  it('notifies on a non-409 delete failure, keeping the row in the list', async () => {
    ...
    const deleteBtn = editBtn.nextElementSibling as HTMLElement
  ...
  it('does not notify on a 409 (in-use) delete rejection — the row is flagged instead', async () => {
    ...
    await screen.findByText('Intake')
    const editBtn = screen.getByRole('button', { name: st('statusList.edit') })
    await user.click(editBtn.nextElementSibling as HTMLElement)
    await user.click(await screen.findByRole('button', { name: i18n.t('confirm', { ns: 'common' }) }))
...
describe('StatusListEditor — readOnly (system value locked)', () => {
  ...
  it('without readOnly the pencil/delete/add stay fully enabled (regression guard)', async () => {
    ...
    expect(editBtn.nextElementSibling as HTMLElement).not.toBeDisabled()
