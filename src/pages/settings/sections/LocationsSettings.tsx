...
function toFormValues(loc: LocationRow): LocationForm {
  const values: Record<string, unknown> = { ...EMPTY_FORM }
...
function formatUsageCounts(counts: Record<string, number> | undefined, t: (key: string, opts?: Record<string, unknown>) => string): string {
...
export default function LocationsSettings() {
  ...
  useEffect(() => {
    ...
    fetchAllPages<LocationRow>('/locations', {}, ctrl.signal)
  ...
  const openEdit = (loc: LocationRow) => { setEditingId(loc.id); setForm(toFormValues(loc)); setShowModal(true) }
  ...
  const inUse = (loc: LocationRow) => Boolean(loc.in_use)
  ...
  const doRemove = async (loc: LocationRow) => {
    ...
    try {
      ...
      const err = e as { response?: { status?: number; data?: { counts?: Record<string, number> } } }
      if (err?.response?.status === 409) {
        // In-use guard: keep the row, flag it, and show WHAT is still linked.
        const list = formatUsageCounts(err.response.data?.counts, t)
