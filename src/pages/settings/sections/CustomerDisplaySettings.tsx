 ...
 * per-entity sub-tabs (Danny 02-08: "departments,
 ...
 *
 * TENANT-DEFAULT-1 (Danny 02-08) update: "Opportunities" was originally DROPPED here
 * because OpportunitiesTab filtered by pipeline stage with no StatusFilterSelect
 * at all — Danny later asked for that filter back ("on Opportunities I'm also
 * still missing the statuses"), so
...
import customerDisplayRaw from '../schemas/customerDisplay'

// The schema module is untyped .js — TS infers its field `type` strings as plain
// `string`, not the Schema union, so we assert the shape once here rather than
// widen SchemaField itself or touch the shared, still-.js schema file.
const customerDisplay = customerDisplayRaw as Schema
...
const tabSchema = (group: string, tab: string) => ({
  ...customerDisplay,
...
function CustomerDisplaySettingsInner() {
  ...
  const setDefaultFilter = (key: string) => (val: string) => saveSettingsKeys({ [key]: val }).catch(err => notifyError(extractApiError(err, t('common:actionFailed'))))
