...
describe('CaoSettings', () => {
  it('loads the list from /cao', async () => {
    mockedApi.get.mockResolvedValue({ data: [cla()] })
    render(<CaoSettings />)

    await screen.findByText('CAO VVT')
    expect(mockedApi.get).toHaveBeenCalledWith('/cao', undefined)
  })

  it('creating a CLA POSTs a slugged value alongside name/label to /cao', async () => {
    mockedApi.get.mockResolvedValue({ data: [cla()] })
    mockedApi.post.mockResolvedValue({ data: cla({ id: 'c2', value: 'ziekenhuizen', label: 'CAO Ziekenhuizen' }) })
    ...
    await waitFor(() => expect(mockedApi.post).toHaveBeenCalledWith('/cao',
  ...
  it('deleting an in-use CLA is blocked, deleting an unused one DELETEs /cao/{id}', async () => {
    mockedApi.get.mockResolvedValue({ data: [cla({ id: 'c1', in_use: false })] })
    ...
    const deleteBtn = editBtn.nextElementSibling as Element
    await user.click(deleteBtn)
    await user.click(await screen.findByRole('button', { name: i18n.t('confirm', { ns: 'common' }) }))

    await waitFor(() => expect(mockedApi.delete).toHaveBeenCalledWith('/cao/c1'))
  })

  it('renaming an existing CLA PUTs the new label to /cao/{id}', async () => {
    mockedApi.get.mockResolvedValue({ data: [cla({ id: 'c1' })] })
    ...
    await waitFor(() => expect(mockedApi.put).toHaveBeenCalledWith('/cao/c1',
