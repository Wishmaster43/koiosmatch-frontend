...
// A fixture WhatsApp message-type row, overridable per test.
interface TypeFixture { id: string; name: string; color: string; is_priority?: boolean; daily_cap?: number }
// eslint-disable-next-line no-restricted-syntax -- DATA: a fixture type's tenant-picked colour, not a style rule.
const type = (over: Partial<TypeFixture> = {}): TypeFixture => ({ id: 'm1', name: 'Sollicitatie', color: '#3B8FD4', ...over })
...
describe('WaMessageTypeSettings — priority flag and daily cap', () => {
  it('POSTs is_priority and daily_cap picked in the create modal', async () => {
    ...
    const [route, body] = api.post.mock.calls[0] as [string, { daily_cap?: unknown }]
  ...
  it('the value mark carries icon and colour (LOOKUP-ICONEN-1)', async () => {
    ...
    await screen.findByText('Sollicitatie')
    const trigger = screen.getByRole('button', { name: st('statusList.valueMark', { label: 'Sollicitatie' }) })
    await user.click(trigger)
    expect(screen.getByRole('menu', { name: st('statusList.valueMark', { label: 'Sollicitatie' }) })).toBeInTheDocument()
  })
})
