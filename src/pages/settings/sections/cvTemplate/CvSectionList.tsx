...
interface CvSectionListProps {
  sections: CvSection[]
  onSave: (patch: { sections: CvSection[] }) => void
}
// `sections` is the normalized list from useCvSettings; `onSave` its partial-save fn.
export default function CvSectionList({ sections, onSave }: CvSectionListProps) {
  ...
  const handleSectionMove = (id: string, dir: number) => {
    const arr = [...sections]
    const target = arr.find(s => s.id === id)
    if (!target) return
    const groupIdx = arr.reduce((acc: number[], s, i) => (s.placement === target.placement ? [...acc, i] : acc), [])
              ...
              <div key={sec.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0',
                ...
                {movable ? (
                  ...
                  <SegmentedControl size="compact" ariaLabel={label} value={sec.placement}
                    onChange={(p: string) => handleSectionPlacement(sec.id, p as CvSectionPlacement)}
                    options={[
