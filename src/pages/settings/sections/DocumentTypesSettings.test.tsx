...
const row = (over: Record<string, unknown> = {}) => ({ id: 'row-1', name: 'CV', color: '#3B8FD4', icon: null, entity: 'candidate', in_use: false, ...over })
...
describe('DocumentTypesSettings — per-entity tab', () => {
  it('GETs scoped by the entity prop', async () => {
    ...
    expect(mockedApi.get).toHaveBeenCalledWith('/document-types', { params: { entity: 'candidate' } })
  })

  it('a different entity prop scopes its own GET (registry mounts one instance per tab)', async () => {
    mockedApi.get.mockResolvedValue({ data: [row({ id: 'row-2', name: 'Contract', entity: 'vacancy' })] })
    render(<DocumentTypesSettings entity="vacancy" />)

    await screen.findByText('Contract')
    expect(mockedApi.get).toHaveBeenCalledWith('/document-types', { params: { entity: 'vacancy' } })
  })

  it('creating a type sends this tab entity + name in the POST body', async () => {
    mockedApi.get.mockResolvedValue({ data: [row()] })
    mockedApi.post.mockResolvedValue({ data: row({ id: 'row-3', name: 'Diploma' }) })
    ...
    await waitFor(() => expect(mockedApi.post).toHaveBeenCalledWith('/document-types', expect.objectContaining({ entity: 'candidate', name: 'Diploma' })))
  })
  ...
  it('editing a type sends its entity in the PUT body', async () => {
    ...
    await waitFor(() => expect(mockedApi.put).toHaveBeenCalledWith('/document-types/row-1', expect.objectContaining({ entity: 'candidate', name: 'CV' })))
  })
  ...
  it('picking a curated icon PUTs it on the row', async () => {
    ...
    await waitFor(() => expect(mockedApi.put).toHaveBeenCalledWith('/document-types/row-1', expect.objectContaining({ icon: 'id-card' })))
  })
  ...
  it('shows the requires_expiry flag badge and validity-months badge on a configured row', async () => {
    mockedApi.get.mockResolvedValue({ data: [row({ requires_expiry: true, default_validity_months: 6 })] })
    render(<DocumentTypesSettings entity="candidate" />)
  ...
  it('creating a type with requires_expiry + a validity POSTs both keys', async () => {
    mockedApi.get.mockResolvedValue({ data: [row()] })
    mockedApi.post.mockResolvedValue({ data: row({ id: 'row-4', name: 'VOG', requires_expiry: true, default_validity_months: 12 }) })
    const user = userEvent.setup()
    render(<DocumentTypesSettings entity="candidate" />)
    ...
    await waitFor(() => expect(mockedApi.post).toHaveBeenCalledWith('/document-types',
  ...
  it('keeps an in-use row on a 409 delete instead of removing it', async () => {
    mockedApi.get.mockResolvedValue({ data: [row({ in_use: false })] })
    ...
    await waitFor(() => expect(mockedApi.delete).toHaveBeenCalledWith('/document-types/row-1'))
  ...
  it('shows the is_cv DefaultToggle on the candidate tab and PUTs is_cv:true on promote', async () => {
    mockedApi.get.mockResolvedValue({ data: [row(), row({ id: 'row-2', name: 'ID-bewijs', is_cv: false })] })
    mockedApi.put.mockResolvedValue({ data: {} })
    const user = userEvent.setup()
    render(<DocumentTypesSettings entity="candidate" />)
