...
it('GenderSettings: row renders the icon-and-colour mark', async () => {
  ...
  const trigger = screen.getByRole('button', { name: st('statusList.valueMark', { label: 'Male' }) })
  expect(trigger).toBeTruthy()

  await user.click(trigger)
  expect(screen.getByRole('menu', { name: st('statusList.valueMark', { label: 'Male' }) })).toBeInTheDocument()
})
