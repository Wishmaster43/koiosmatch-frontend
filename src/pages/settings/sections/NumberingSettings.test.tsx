...
// Mocked axios instance behind the dynamic import, typed as its mocked methods.
type MockedApi = { get: ReturnType<typeof vi.fn>; post: ReturnType<typeof vi.fn>; patch: ReturnType<typeof vi.fn>; delete: ReturnType<typeof vi.fn> }
async function mockedApi(): Promise<MockedApi> {
  const apiModule = await import('@/lib/api')
  return apiModule.default as unknown as MockedApi
}

// A backend-configured numbering entity row (GET /numbering-entities shape).
interface EntityFixture { key: string; prefix: string; pad: number; start: number; label: string; allocated: boolean; next_value?: number }

const TWELVE: EntityFixture[] = [
...
async function renderNumbering(entitiesResponse: EntityFixture[] | Error) {
  vi.resetModules()
  const apiModule = await mockedApi()
  apiModule.get.mockImplementation((url: string) => {
...
describe('NumberingSettings — renders the backend entity list, not a hardcoded six', () => {
  ...
  it('renders editable start input when allocated is false (existing behaviour)', async () => {
    ...
    const startInputs = screen.getAllByDisplayValue('1').filter(input => (input as HTMLInputElement).type === 'number')
...
describe('NumberingSettings — B-35 prefix validation (alphanumeric, max 10)', () => {
  ...
  it('saves a valid prefix through POST /settings with the exact numbering key', async () => {
    ...
    await waitFor(() => expect(apiModule.post).toHaveBeenCalledWith('/settings', { 'numbering.candidate.prefix': 'KX' }))
