...
// GET/PUT /admin/platform-pricing response shape (the USD→EUR rate the API still
// returns is deliberately unread here — see the file header, Danny 09-09).
interface PlatformPricing { ai_markup_percent?: string | number }
...
export default function PlatformPricingCard() {
  ...
  const load = () => {
    ...
    return api.get('/admin/platform-pricing')
      .then((res) => {
        const d = unwrap<PlatformPricing>(res) ?? {}
