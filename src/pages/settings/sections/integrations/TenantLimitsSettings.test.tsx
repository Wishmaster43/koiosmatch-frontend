...
const rows = [
  { key: 'ai', label: 'Koios AI-tokens', scope: 'tenant', window: 'month', used: 12000, cap: 50000, percent: 24, cap_reached: false,
    prices: { tier: { key: 'smart', label: 'Slim', monthly_tokens: 50000, price_cents: 4900, effective_from: '2026-09-01', source: 'chosen' } } },
...
describe('TenantLimitsSettings', () => {
  ...
  it('shows the billing tier line only for a row that carries prices', async () => {
    vi.mocked(api.get).mockResolvedValueOnce({ data: { data: rows } })
    renderPage()
