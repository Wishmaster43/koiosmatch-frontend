...
async function renderWithSources(sourceRows: Array<Record<string, unknown>> = [], allowFreeEntry = false) {
  vi.resetModules()
  const apiModule = await import('@/lib/api')
  // The mocked-module factory above hands back plain vi.fn()s — vi.mocked() gives
  // every call site (here and every caller of renderWithSources) their real Mock typing.
...
describe('ApplicationSourcesSettings — wired to the REAL /candidate-sources lookup', () => {
  ...
  it('deletes a source that is not in use via DELETE /candidate-sources/{id}', async () => {
    ...
    await user.click(editBtn.nextElementSibling as Element)
    await user.click(await screen.findByRole('button', { name: confirmBtnName }))
