...
// The one JSON tenant setting's shape (SETTINGS_KEY, shared contract with the propose-candidate modal).
interface ProposalSettingsBlob {
  subject_template: string
  body_template: string
  sets_phase: boolean
  default_cv_variant: 'proposal' | 'full'
}

const DEFAULTS: ProposalSettingsBlob = {
  subject_template: '',
  body_template: '',
  sets_phase: false,
...
export default function ProposalSettings() {
  ...
  const senderOptions = users.map(u => ({ value: String(u.id ?? ''), label: u.name ?? String(u.id ?? '') }))

  // VOORSTEL-AFZENDER-FE-1: the stored default sender, '' meaning "the proposer".
  const defaultSenderId = typeof values[SENDER_SETTING_KEY] === 'string' ? (values[SENDER_SETTING_KEY] as string) : ''
  ...
  const chooseSender = async (id: string | number | null) => {
    try {
  ...
  // SegmentedControl's onChange is (value: string) => void; the two literal values are the only ones it ever offers.
  const chooseVariant = (variant: string) => {
    if (variant === persisted.default_cv_variant) return
    saveSettingsKeys({ [SETTINGS_KEY]: { ...persisted, default_cv_variant: variant as ProposalSettingsBlob['default_cv_variant'] } })
      .then(invalidateAllSettingsCache)
      .catch(() => notifyError(t('proposal.saveFailed')))
  }

  const cardStyle: CSSProperties = { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', marginBottom: 12 }
  const labelStyle: CSSProperties = { fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 4, display: 'block' }
  const hintStyle: CSSProperties = { fontSize: 11, color: 'var(--text-muted)', marginBottom: 10 }
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <div style={cardStyle}>
        ...
        <input id="proposal-subject" value={subject} onChange={(e: ChangeEvent<HTMLInputElement>) => setSubject(e.target.value)}
