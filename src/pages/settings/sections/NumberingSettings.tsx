...
import { useNumberingEntities, type NumberingEntity } from '@/lib/useNumberingEntities'
...
const th: CSSProperties = { padding: '10px 12px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }

// The whole tenant settings blob (flat key/value store).
...
function EntityRow({ entity, settings }: EntityRowProps) {
  ...
  const savedPrefix = typeof settings?.[keys.prefix] === 'string' ? (settings[keys.prefix] as string) : entity.prefix
  ...
  const isValidPrefix = (val: string) => /^[a-zA-Z0-9]*$/.test(val) && val.length <= 10

  // Persist one field — optimistic, revert + toast on failure (house pattern).
  const commit = async <T extends string | number>(key: string, value: T, prevValue: T, setter: (v: T) => void) => {
    ...
    <tr style={{ borderBottom: '1px solid var(--border)' }}>
      ...
      <td style={{ padding: '8px 12px' }}>
        <input type="number" min={1} max={8} value={pad} aria-label={t('numbering.pad')}
          onChange={(e: ChangeEvent<HTMLInputElement>) => setPad(Number(e.target.value))}
          onBlur={(e: FocusEvent<HTMLInputElement>) => commit(keys.pad, Math.min(8, Math.max(1, Number(e.target.value) || entity.pad)), savedPad, setPad)}
      ...
      <td style={{ padding: '8px 12px' }}>
        {entity.allocated ? (
          ...
          <input type="number" min={1} value={start} aria-label={t('numbering.start')}
            onChange={(e: ChangeEvent<HTMLInputElement>) => setStart(Number(e.target.value))}
            onBlur={(e: FocusEvent<HTMLInputElement>) => commit(keys.start, Math.max(1, Number(e.target.value) || entity.start), savedStart, setStart)}
