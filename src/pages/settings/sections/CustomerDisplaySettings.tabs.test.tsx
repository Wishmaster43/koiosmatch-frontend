...
vi.mock('@/lib/api', () => ({
  default: {
    get: vi.fn((url: string) => (url === '/settings' ? Promise.resolve({ data: {} }) : new Promise(() => {}))),
...
const tabLabel = (id: string) => t(`customerDisplay.tabs.${id}.title`)
...
describe('CustomerDisplaySettings · every one of the eleven keys is still reachable', () => {
  it.each(customerDisplay.fields.map((f: { key: string; group?: string }) => [f.key, f.group]))('%s (group: %s) renders its label on its own tab', async (key: string, group: string | undefined) => {
    const user = userEvent.setup()
    render(<CustomerDisplaySettings />)
    if (group !== 'customer_table') {
      await user.click(screen.getByRole('tab', { name: tabLabel(group as string) }))
    }
