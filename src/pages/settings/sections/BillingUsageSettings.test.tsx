/**
 * BillingUsageSettings (F5) — asserts the sub-tab bar, the ONE shared period param
...
// Captures the real FilterGroup config BillingUsageSettings registers, so a test
...
function renderPage() {
  return render(<RightPanelProvider><BillingUsageSettings /></RightPanelProvider>)
}

beforeEach(() => { window.history.replaceState({}, '', '#billing_usage') })
afterEach(() => vi.clearAllMocks())

describe('BillingUsageSettings — sub-tabs share one period', () => {
  ...
  it('the period pick reaches BOTH /billing/usage and the Per-functie /ai/koios/usage fetch', async () => {
    ...
    // Drive the pick through the real onToggle BillingUsageSettings registered —
...
describe('BillingUsageSettings — per-workflow row navigates to the workflow editor', () => {
...
describe('BillingUsageSettings — Per gebruiker renders ai.per_user with success_rate as a RATIO', () => {
