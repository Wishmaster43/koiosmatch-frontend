...
import type { FreeEntryMismatch } from './FreeEntryMismatchDialog'

// The lookup hook's returned shape (useFunctions/useApplicationSources/…): only the two fields this screen reads.
interface FreeEntryLookup { allowFreeEntry: boolean; invalidate: () => void }
interface FreeEntryLookupSettingsProps {
  useLookup: () => FreeEntryLookup
  endpoint: string
  i18nPrefix: string
  strictPreflight?: boolean
  statusListEditorProps?: Partial<StatusListEditorProps>
}
...
export default function FreeEntryLookupSettings({ useLookup, endpoint, i18nPrefix, strictPreflight = false, statusListEditorProps = {} }: FreeEntryLookupSettingsProps) {
  ...
  const [mismatches, setMismatches] = useState<FreeEntryMismatch[] | null>(null)
