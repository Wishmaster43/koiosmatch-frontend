...
// One non-conforming record the 409 preflight reported (a candidate function no longer in the list, etc.).
export interface FreeEntryMismatch {
  id?: string | number
  name?: string
  entity?: string
  function?: string
...
export function mismatchesFromError(e: unknown): FreeEntryMismatch[] | null {
  const resp = (e as { response?: { status?: number; data?: { mismatches?: unknown } } } | undefined)?.response
  if (resp?.status !== 409 || !Array.isArray(resp.data?.mismatches)) return null
  return resp.data.mismatches as FreeEntryMismatch[]
}

interface FreeEntryMismatchDialogProps {
  mismatches: FreeEntryMismatch[] | null
  onClose: () => void
}
// The strict-mode preflight dialog; renders nothing without a mismatches list.
export default function FreeEntryMismatchDialog({ mismatches, onClose }: FreeEntryMismatchDialogProps) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 260, overflowY: 'auto' }}>
        {mismatches.map((m, i) => (
          <div key={m.id ?? i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px',
            ...
            {(m.count ?? 0) > 1 && <Caption>×{m.count}</Caption>}
          </div>
        ))}
      </div>
