...
// canAccessPage's own auth shape isn't exported — this local minimal type mirrors
// only what these fixtures set (activeTenant/user with a modules[] list).
type AuthFixture = { activeTenant: { modules: string[] }; user: { tenant: { modules: string[] } } }

// Mirrors SettingsPage.jsx's `visibleGroups` memo.
function visibleGroups(auth: AuthFixture) {
  const authArg = auth as Parameters<typeof canAccessPage>[1]
  return NAV_GROUPS
    .map(group => ({ ...group, items: group.items.filter(it => !it.requiresPage || canAccessPage(it.requiresPage, authArg)) }))
