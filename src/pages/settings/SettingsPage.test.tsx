...
describe('passesModuleOrApp — module/app OR-gate', () => {
  it('no requirement at all → always visible', () => {
    expect(passesModuleOrApp(undefined, { hasModule: () => false, isAppEnabled: () => false })).toBe(true)
  })

  it('module only → visible', () => {
    const hasModule = (k: string) => k === 'sm'
  ...
  it('app only → visible', () => {
    ...
    const isAppEnabled = (k: string) => k === 'shiftmanager'
    expect(passesModuleOrApp(gate, { hasModule, isAppEnabled })).toBe(true)
  })
