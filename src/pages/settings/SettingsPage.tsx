...
// requirement: { module?, app? } — from a registry nav item's requiresModuleOrApp field.
interface ModuleOrAppRequirement { module?: string; app?: string }
...
export function passesModuleOrApp(
  requirement: ModuleOrAppRequirement | undefined,
  { hasModule, isAppEnabled }: { hasModule: (m: string) => boolean; isAppEnabled?: (a: string) => boolean },
): boolean {
...
const SLUG_ALIASES: Record<string, { category: string; tab: string }> = {
...
export default function SettingsPage() {
  // Settings only ever renders inside AuthProvider (mirrors the original JS, which
  // never null-checked auth here either) — the non-null assertion documents that.
  ...
  const findLocation = (groupKey: string | null, tabId: string | null) => {
  ...
  const dirtyCtx = useMemo(() => ({ report: (d: boolean) => { dirtyRef.current = d } }), [])
  ...
  useEffect(() => {
    const redirectIfMoved = () => {
      const loc = parseHash()
      const moved = loc ? (MOVED_TO_PROFILE as Record<string, unknown>)[`${loc.category}/${loc.tab}`] : null
  ...
  const applyNav = (groupKey: string, tabId: string) => {
  ...
  const goTo = (groupKey: string, tabId: string, { guard = true }: { guard?: boolean } = {}) => {
  ...
  // out of this migration's scope) — cast past its inferred null-only type here.
  return (
    <SettingsDirtyContext.Provider value={dirtyCtx as never}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <SettingsTabs items={currentGroup.items} active={tab}
                    onSelect={(id: string) => goTo(category!, id)}
