...
import { MapPin, Building2, Layers, Phone, Mail, ChevronRight } from 'lucide-react'
...
export default function LocationDrawer({ loc, onClose }: { loc: SmLocationRow | null; onClose: () => void }) {
  ...
  return (
    <SmDrawerShell title={t('locationsPage.drawerTitle')} onClose={onClose}
      // §3A(d): record history is a changelog icon-popover in the title row, never a tab.
      headerExtras={loc.id !== undefined ? (
        ...
        <SmStatCardGrid
          cards={[
            ...
            { label: t('locationsPage.drawer.activeShifts'), value: loc.shifts ?? 0, Icon: Building2, color: 'var(--color-success-text)', bg: 'var(--color-success-bg)' },
          ]}
        />
