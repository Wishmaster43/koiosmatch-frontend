...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ fontSize: 11, fontWeight: 500, padding: '3px 9px', borderRadius: 999,
      ...
      color:      active ? '#16A34A' : '#6B7280', whiteSpace: 'nowrap' }}>
      {status || 'Onbekend'}
    </span>
  )
}
    ...
    <div style={{ width: 380, flexShrink: 0, borderLeft: '1px solid var(--border)',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
          {[
            { label: 'Afdelingen', value: loc.departments.length, icon: Layers, color: '#6366F1', bg: '#EEF2FF' },
            { label: 'Actieve diensten', value: loc.shifts, icon: Building2, color: '#22C55E', bg: '#DCFCE7' },
          ].map(s => (
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            <button style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#6366F1',
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 24px 18px', display: 'flex', gap: 20, flexShrink: 0 }}>
          {[
            { label: 'Totaal locaties',  value: locations.length,                                    color: '#6366F1', bg: '#EEF2FF', Icon: MapPin },
            { label: 'Actief',           value: locations.filter(l => l.status === 'Actief').length, color: '#22C55E', bg: '#DCFCE7', Icon: Building2 },
            { label: 'Afdelingen',       value: locations.reduce((s,l) => s + l.departments.length, 0), color: '#F59E0B', bg: '#FEF3C7', Icon: Layers },
            { label: 'Gekoppelde klanten', value: [...new Set(locations.map(l => l.customer))].length,  color: '#3B82F6', bg: '#EFF6FF', Icon: Building2 },
          ].map(k => (
