    ...
    <div style={{ width: 380, flexShrink: 0, borderLeft: '1px solid var(--border)',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
          {[
            { label: 'Medewerkers', value: dep.employees, Icon: Users,    color: '#6366F1', bg: '#EEF2FF' },
            { label: 'Diensten',    value: dep.shifts,    Icon: Layers,   color: '#22C55E', bg: '#DCFCE7' },
          ].map(s => (
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 24px 18px', display: 'flex', gap: 20, flexShrink: 0 }}>
          {[
            { label: 'Totaal afdelingen',    value: departments.length,                                       color: '#6366F1', bg: '#EEF2FF', Icon: Layers },
            { label: 'Actief',               value: departments.filter(d => d.status === 'Actief').length,   color: '#22C55E', bg: '#DCFCE7', Icon: Layers },
            { label: 'Medewerkers',          value: departments.reduce((s,d) => s + d.employees, 0),          color: '#F59E0B', bg: '#FEF3C7', Icon: Users },
            { label: 'Gekoppelde klanten',   value: [...new Set(departments.map(d => d.customer))].length,   color: '#3B82F6', bg: '#EFF6FF', Icon: Building2 },
          ].map(k => (
