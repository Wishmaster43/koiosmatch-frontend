/**
 * DrawerAddApplicationModal — the candidate-drawer context of the merged
 * AddApplicationModal (ADDAPPLICATION-TWIN-1): "+ Solliciteren" from the
 * candidate Match tab, couple the candidate to a vacancy in a chosen funnel
 * phase. vacancy_id is nullable server-side (APP-VACANCY-OPTIONAL-1) — an open
 * application without a vacancy yet is a real backend flow, so the picker is
 * optional and Create never waits on it. Vacancy + phase + owner + source are
 * all searchable pick-only comboboxes (S24b), owner seeded from the
 * APP-OWNER-1 priority chain (picked vacancy's recruiter > candidate's own
 * owner > logged-in user) with an OWNER-DEVIATION-1 soft warning when the
 * final pick still differs — both the chain and the soft-warning booleans
 * live in useApplicationOwnerChain.
 *
 ...
 * (StoreApplicationRequest accepts both), mirroring PageAddApplicationModal's
 * identical fields.
 *
 * ADDAPPLICATION-TWIN-1 (relocation, frozen behaviour): this component and its
 * two hooks (useApplicationModalLookups/useAddApplicationForm) moved verbatim
 * from pages/candidates/drawer + pages/candidates/hooks into
 * pages/applications — pages/candidates/drawer/AddApplicationModal.tsx is now
 * a thin adapter that renders the merged pages/applications/AddApplicationModal
 * with context="drawer", forwarding exactly the props it always took. The
 * candidate drawer's look and behaviour are unchanged (§14 SCHERMWAARHEID-1);
 * only the "Extra" custom-fields block now reuses the applications addmodal/
 * CustomFieldsSection (byte-identical to the old local
 * ApplicationCustomFieldsSection, `hasError` simply omitted here as before).
 */
    ...
    <FloatingPanel open onClose={onClose} title={title} ariaLabel={title}
      ...
      <div style={{ overflow: 'auto', flex: 1, minHeight: 0, padding: 22 }}>
        ...
        {/* W30: tenant custom fields — only rendered once ≥1 active def exists (§3A(f)).
            ...
            stays omitted here, exactly as before the merge). */}
