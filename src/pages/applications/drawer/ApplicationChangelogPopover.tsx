...
/**
 * ApplicationChangelogPopover — a History icon in the drawer title-row that opens
 * the application's audit trail in a popover. Mirrors the candidate/opportunity/
 * match ChangelogPopover: record history stays one click from every tab instead
 * of being its own tab (§3A).
 */
export default function ApplicationChangelogPopover({ application }: { application: ApplicationDetail }) {
      ...
      {open && (
        <div role="dialog" aria-label={t('drawer.tabs.changelog')}
          ...
          <div style={{ overflowY: 'auto', padding: '12px 14px' }}>
            <ChangelogTab application={application} />
          </div>
        </div>
      )}
    </div>
