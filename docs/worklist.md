...
| **F-8** | ◐ | L | FE | Inline `api.*`+`useEffect` → feature-hooks (§3). **Voortgang: 49 → 33** (alle lanes). ✅ A1: customer-drawer ×4 · Matches · Opportunities · A2: UsersPage (`b230e0f`) · B: candidates ✅ · C: SM-reports (`01066b4`). **Rest: A1 9 · A2 9 · C 14** — mutatie-containers = volledige container-hook, 1/turn. Teller onderhouden door Instance-X (deze regel) → andere lanes raken dit bestand niet. |
