...
| **F-8** | ◐ | L | FE | Componenten fetchen inline (`api.get`+`useEffect`) → naar feature-hooks (§3). **Voortgang (Lane A): 49 → 43** — customer-drawer ×4 (`useCustomerDrawerData`) · MatchesPage (`useMatches`) · OpportunitiesPage (`useOpportunitiesData`). Mutatie-containers = volledige container-hook, 1/turn + browser-check. **candidates → Claude B · shiftmanager/reports → Claude C.** |
